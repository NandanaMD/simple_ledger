...
import {
  ...
  getDatabasePath,
  initializeDatabase,
} from '../db/DatabaseClient.js';
import type { BackupResult, RestoreResult } from '../../shared/types/ledger.js';

const AUTO_BACKUP_MIN_SLOT = 6;
const AUTO_BACKUP_MAX_SLOT = 10;
const DESKTOP_BACKUP_FOLDER_NAME = 'Ledger Backup';

const getDesktopBackupDirectory = (): string =>
  path.join(app.getPath('desktop'), DESKTOP_BACKUP_FOLDER_NAME);

const getAutoBackupDirectories = (): string[] => {
  const directories = [getDesktopBackupDirectory(), app.getPath('documents')];
  return [...new Set(directories)];
};

const getSlotPath = (directory: string, slot: number): string =>
  path.join(directory, `SL_backup_${slot}.db`);

const rotateBackupSlots = async (directory: string): Promise<void> => {
...
const getDefaultBackupPath = (): string => {
  const now = new Date();
...
export const backupDatabase = async (): Promise<BackupResult> => {
...
export const restoreDatabase = async (): Promise<RestoreResult> => {
  const parentWindow = BrowserWindow.getFocusedWindow() ?? BrowserWindow.getAllWindows()[0] ?? null;
