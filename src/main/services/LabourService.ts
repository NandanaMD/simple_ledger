...
const mapLabourerSummary = (row: LabourerSummaryRow): LabourerSummary => ({
  id: row.id,
  name: row.name,
  notes: row.notes,
...
const mapLabourEntry = (row: LabourEntryRow): LabourEntry => ({
  id: row.id,
...
const mapPeriodSummary = (row: PeriodSummaryRow): PeriodSummary => ({
...
const validateLabourerInput = (payload: LabourerInput): void => {
  if (!payload.name.trim()) {
    throw new Error('Labourer name is required.');
  }
};

const validateEntryInput = (payload: LabourEntryInput): void => {
...
const normalizePaymentAccountId = (value?: number): number | null => {
  if (!value || value <= 0) {
    return null;
  }
  return value;
...
const ensureLabourerExists = (labourerId: number): void => {
  const database = getDatabase();
  const row = database
    .prepare('SELECT id FROM labourers WHERE id = ?;')
    .get(labourerId) as { id: number } | undefined;
  if (!row) {
    throw new Error('Labourer not found.');
  }
};

const ensureBankAccountForPayout = (
  accountId: number,
  amount: number,
...
const getLabourerSummaryById = (id: number): LabourerSummary => {
  const database = getDatabase();
  const row = database
    .prepare(
      `SELECT
    ...
    .get(id) as LabourerSummaryRow | undefined;
...
const getLabourEntryById = (id: number): LabourEntry => {
  const database = getDatabase();
  const row = database
    .prepare(
      `SELECT
    ...
    .get(id) as LabourEntryRow | undefined;
...
const syncLinkedExpenseTransaction = (
...
export const listLabourers = async (): Promise<LabourerSummary[]> => {
  const database = getDatabase();
  const rows = database
    .prepare(
      `SELECT
    ...
    .all() as LabourerSummaryRow[];

  return rows.map(mapLabourerSummary);
};

export const createLabourer = async (payload: LabourerInput): Promise<LabourerSummary> => {
...
export const updateLabourer = async (id: number, payload: LabourerInput): Promise<LabourerSummary> => {
  validateLabourerInput(payload);
  const database = getDatabase();
  const result = database
    .prepare(
...
export const listLabourEntries = async (labourerId?: number): Promise<LabourEntry[]> => {
  const database = getDatabase();
  ...
  const rows = labourerId
    ? (database
        ...
        .all(labourerId) as LabourEntryRow[])
    : (database
        .prepare(
          `SELECT
            e.id,
        ...
        .all() as LabourEntryRow[]);

  return rows.map(mapLabourEntry);
};

export const createLabourEntry = async (payload: LabourEntryInput): Promise<LabourEntry> => {
  ...
  const labourer = database
    ...
    .get(payload.labourerId) as { name: string };

  const linkedTransactionId = syncLinkedExpenseTransaction(
    null,
    paymentAccountId,
    payload.entryType,
    payload.amount,
...
export const updateLabourEntry = async (id: number, payload: LabourEntryInput): Promise<LabourEntry> => {
  validateEntryInput(payload);
  ensureLabourerExists(payload.labourerId);
  ...
  const existing = database
    ...
    .get(id) as { id: number; linked_transaction_id: number | null } | undefined;
...
export const deleteLabourEntry = async (id: number): Promise<void> => {
  const database = getDatabase();
  ...
  const existing = database
    ...
    .get(id) as { linked_transaction_id: number | null } | undefined;
...
const getPeriodSummary = (periodExpression: string, labourerId?: number): PeriodSummary[] => {
  const database = getDatabase();
  ...
  const rows = labourerId
    ? (database.prepare(sql).all(labourerId) as PeriodSummaryRow[])
    : (database.prepare(sql).all() as PeriodSummaryRow[]);

  return rows.map(mapPeriodSummary);
};

export const getLabourSummaries = async (labourerId?: number): Promise<LabourSummaryResponse> => {
  if (labourerId !== undefined) {
    ensureLabourerExists(labourerId);
  }
