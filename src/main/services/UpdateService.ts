...
import type { UpdaterStatus } from '../../shared/types/update.js';
...
const sendStatus = (status: UpdaterStatus): void => {
  latestStatus = status;
...
export const initializeUpdater = (window: BrowserWindow): void => {
  mainWindow = window;
...
export const checkForAppUpdates = async (): Promise<UpdaterStatus> => {
  if (!app.isPackaged) {
...
export const downloadAppUpdate = async (): Promise<UpdaterStatus> => {
  if (!app.isPackaged) {
    const status: UpdaterStatus = {
      state: 'unsupported',
      message: 'Auto update is available only in installed production builds.',
...
export const installDownloadedUpdate = (): void => {
  if (!app.isPackaged) {
    sendStatus({
      state: 'unsupported',
      message: 'Auto update is available only in installed production builds.',
...
export const getUpdaterStatusSnapshot = (): UpdaterStatus => latestStatus;
