...
const fetchTransactions = (period) => {
...
const fetchLabourEntries = (period) => {
    const database = getDatabase();
    const startDate = getDateStart(period);
    const rows = (startDate
        ? database
...
const fetchData = (dataset, period) => {
...
const buildCsvContent = (document) => {
...
const saveBufferAsPdf = async (filePath, html) => {
...
export const openExportFile = async (filePath) => {
